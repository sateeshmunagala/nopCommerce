...
public class InterviewRuntimeService : IInterviewRuntimeService
{
    ...
    public async Task<PrepareInterviewResponseModel> PrepareInterviewAsync(string token, Customer customer = null)
    {
        ...
        var lazyTask = PreparationTasks.GetOrAdd(session.Id, _ => new Lazy<Task<PrepareInterviewResponseModel>>(
            () => PrepareInterviewCoreAsync(token, customer),
        ...
        finally
        {
            if (lazyTask.IsValueCreated && lazyTask.Value.IsCompleted)
                PreparationTasks.TryRemove(new KeyValuePair<int, Lazy<Task<PrepareInterviewResponseModel>>>(session.Id, lazyTask));
        }
    }

    private async Task<PrepareInterviewResponseModel> PrepareInterviewCoreAsync(string token, Customer customer = null)
    {
        ...
        int maxQuestions;
        IList<InterviewTurn> preparedTurns;

        using (await AcquireSessionMutationLockAsync(sessionId))
        {
            session = await _sessionService.GetInterviewSessionByIdAsync(sessionId) ?? await _sessionService.GetSessionByTokenAsync(token);
            if (!IsSessionUsable(session, DateTime.UtcNow))
                return null;
        ...
        var missingSequences = Enumerable.Range(1, maxQuestions)
            .Where(sequenceNumber => !preparedTurns.Any(turn => turn.SequenceNumber == sequenceNumber))
            .ToList();
        var generatedPlan = await GenerateQuestionPlanTurnsAsync(session, customer, preparedTurns, missingSequences);
        ...
        using (await AcquireSessionMutationLockAsync(session.Id))
        {
            ...
            maxQuestions = GetMaxQuestions(session);
            var latestTurns = (await _turnService.GetTurnsBySessionIdAsync(session.Id))
                .OrderBy(turn => turn.SequenceNumber)
                .ThenBy(turn => turn.Id)
                .ToList();
            var existingSequenceNumbers = latestTurns.Select(turn => turn.SequenceNumber).ToHashSet();
            if (generatedPlan.Turns.Any())
            {
                foreach (var turn in generatedPlan.Turns
                    .Where(turn => turn.SequenceNumber >= 1 && turn.SequenceNumber <= maxQuestions)
                    .OrderBy(turn => turn.SequenceNumber))
                {
                    if (existingSequenceNumbers.Contains(turn.SequenceNumber))
                        continue;
    ...
    public async Task<SubmitInterviewAnswerResponse> SubmitAnswerAsync(SubmitInterviewAnswerRequest request)
    {
        ...
        var completion = await AcceptTerminalAnswerAndQueueCompletionAsync(session, turns, currentTurn, evaluation.Completion ?? evaluation.Feedback ?? evaluation.ErrorMessage, evaluation.Completion);
        return new SubmitInterviewAnswerResponse
        {
            Success = true,
            IsTerminated = true,
        ...
        Stopwatch submitStopwatch)
    {
        ...
        var completion = await AcceptTerminalAnswerAndQueueCompletionAsync(session, turns, currentTurn, "Interview completed.");
        return new SubmitInterviewAnswerResponse
        {
    ...
    protected virtual async Task<CompleteInterviewResponse> AcceptTerminalAnswerAndQueueCompletionAsync(
        InterviewSession session,
        IList<InterviewTurn> turns,
        InterviewTurn currentTurn,
        string reason,
        ...
        bool persistCurrentTurn = false)
    {
        ...
        if (acceptedNow)
        {
            ...
            await LogRuntimeIssueAsync(
                ...
                $"Mode=completion-pending; SessionId={session.Id}; ProductId={session.ProductId}; CustomerId={session.CustomerId}; TurnId={currentTurn?.Id ?? 0}; SequenceNumber={currentTurn?.SequenceNumber ?? 0}; ExpectedQuestions={maxQuestions}; AnsweredTurns={completedTurns.Count}.",
                await ResolveLogCustomerAsync(session));
            await LogRuntimeActivityAsync(
                session,
                "AIInterview.Runtime.CompletionAccepted",
                BuildRuntimeActivityComment(session, currentTurn, CompletionPendingMessage),
                entity: currentTurn);
        }
        else
        {
            await LogRuntimeIssueAsync(
                NopLogLevel.Information,
                "AI Interview completion duplicate suppressed",
    ...
    private async Task<SubmitInterviewAnswerResponse> BuildPendingCompletionSubmitResponseAsync(InterviewSession session, IList<InterviewTurn> turns)
    {
        ...
        await LogRuntimeIssueAsync(
            NopLogLevel.Information,
            "AI Interview completion duplicate suppressed",
            $"Mode=submit-after-pending; SessionId={session?.Id ?? 0}; ProductId={session?.ProductId ?? 0}; CustomerId={session?.CustomerId ?? 0}; AnsweredTurns={completedTurns.Count}.",
            await ResolveLogCustomerAsync(session));
        ...
        return new SubmitInterviewAnswerResponse
        {
            ...
            Feedback = completedTurns.LastOrDefault()?.Feedback ?? string.Empty,
            Message = CompletionPendingMessage,
            Completion = string.Empty,
            ReportUrl = string.Empty,
            ReportReady = false,
    ...
    protected virtual async Task<CompleteInterviewResponse> CompleteInterviewInternalAsync(InterviewSession session, IList<InterviewTurn> turns, string reason, string aiCompletion = null)
    {
        ...
        if (session.CompletedOnUtc.HasValue || (!session.IsActive && !IsCompletionPending(session)))
        {
            await LogRuntimeActivityAsync(
                session,
