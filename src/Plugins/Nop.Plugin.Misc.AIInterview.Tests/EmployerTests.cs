...
public class EmployerTests
{
    ...
    public void EmployerDashboard_Partials_Use_Pagers_Compact_Actions_And_DateOnly_Invites()
    {
        ...
        Assert.That(invitesPartial, Does.Contain("<th class=\"col-invite-email\">@T(\"Plugins.Misc.AIInterview.Employer.Invite.Email\")</th>"));
        ...
        Assert.That(invitesPartial.Split("<th class=\"col-invite-").Length - 1, Is.EqualTo(6));
        ...
        Assert.That(invitesPartial, Does.Contain("FormatInviteDateTimeAsync(invite.ExpiryDateUtc, includeSeconds: true)"));
        Assert.That(invitesPartial, Does.Contain("includeSeconds ? \"G\" : \"g\""));
        Assert.That(invitesPartial, Does.Contain("<td class=\"col-invite-email\"><span class=\"employer-invite-email\">@invite.Email</span></td>"));
        Assert.That(invitesPartial, Does.Not.Match(@"(?s)<td class=""col-invite-job"">(?:(?!</td>).)*@invite\.Email"));
        Assert.That(invitesPartial, Does.Contain("responsive-data-card-label\">@T(\"Plugins.Misc.AIInterview.Employer.Invite.Email\")"));
        Assert.That(invitesPartial, Does.Contain("responsive-data-card-value employer-invite-email\">@invite.Email"));
        ...
        Assert.That(squareTableRule, Does.Contain("border-radius: 0 !important;"));
        Assert.That(cssText, Does.Contain(".html-aiinterview-employer-dashboard-page .employer-jobs-table-shell,"));
        Assert.That(cssText, Does.Contain(".html-aiinterview-employer-dashboard-page .employer-applications-table-shell,"));
        Assert.That(cssText, Does.Contain(".html-aiinterview-employer-dashboard-page .employer-invites-table-shell,"));
        Assert.That(cssText, Does.Contain(".html-aiinterview-employer-dashboard-page .scoreboard-deck-table th,"));
        Assert.That(cssText, Does.Contain(".html-aiinterview-employer-dashboard-page .employer-table th,"));
        Assert.That(cssText, Does.Contain(".html-aiinterview-employer-dashboard-page .invite-table th {"));
        ...
        Assert.That(localeResources["Plugins.Misc.AIInterview.Employer.Invite.Email"], Is.EqualTo("Email"));
